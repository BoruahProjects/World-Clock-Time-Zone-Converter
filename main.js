...
function initTheme() {
  ...
  saved === DARK ? removeBubbles() : createBubbles();
}
function toggleTheme() {
  ...
  !isDark ? removeBubbles() : createBubbles();
}
...
function setupStarryBackground() {
    ...
    function randomColor() {
      const palette = [
        ...
        [100, 100, 255], [255, 255, 100], [255, 180, 255], [180, 255, 255],
      ];
      return palette[Math.floor(Math.random() * palette.length)];
    }
    function createStar() {
      return { x: Math.random()*canvas.width, y: Math.random()*canvas.height, r: Math.random()*1.2+0.3,
        color: randomColor(), age:0, fadeIn: Math.random()*30+20, full: Math.random()*50+40,
        fadeOut: Math.random()*30+20, delay: Math.random()*500 };
    }
    for (let i=0;i<starCount;i++) stars.push(createStar());
  
    const prefersReduced = matchMedia("(prefers-reduced-motion: reduce)");
    let running = !prefersReduced.matches && !document.hidden;
    document.addEventListener("visibilitychange", () => running = !document.hidden && !prefersReduced.matches);
    prefersReduced.addEventListener?.("change", e => running = !e.matches && !document.hidden);
  
    function draw() {
      if (running) {
        ctx.clearRect(0,0,canvas.width,canvas.height);
        stars.forEach((star,i) => {
          star.age++; if (star.age < star.delay) return;
          const t = star.age - star.delay;
          let opacity = 0;
          if (t < star.fadeIn) opacity = t / star.fadeIn;
          else if (t < star.fadeIn + star.full) opacity = 1;
          else if (t < star.fadeIn + star.full + star.fadeOut) opacity = 1 - (t - star.fadeIn - star.full) / star.fadeOut;
          else stars[i] = createStar();
          const [r,g,b] = star.color;
          ctx.beginPath(); ctx.arc(star.x, star.y, star.r, 0, Math.PI*2);
          ctx.fillStyle = `rgba(${r},${g},${b},${opacity.toFixed(2)})`; ctx.fill();
        });
      }
